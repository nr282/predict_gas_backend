...
def solve_dataframe(df: pd.DataFrame,
    ...
    column_name_to_function_id, function_column_names, aggregate_column_names = create_column_dictionaries(
        column_mapping)
    ...
    df["Days_Between_Global_Start_Date_And_Period_Start_Date"] = df["Period_Start_Date"].apply(
        lambda dt: (dt - global_start_date).days)
    df["Days_Between_Global_Start_Date_And_Period_End_Date"] = df["Period_End_Date"].apply(
        lambda dt: (dt - global_start_date).days)
